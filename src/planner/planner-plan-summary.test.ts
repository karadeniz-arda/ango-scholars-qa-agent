...
test("summary exposes trusted, discovery, and effective runtime counts", () => {
  ...
  assert.equal(summary.trustedBrowserCaseCount, 0);
  assert.equal(summary.discoveryBrowserCaseCount, 4);
  assert.equal(summary.effectiveBrowserRuntimeCaseCount, 4);
  assert.equal(summary.effectiveTotalRuntimeUnits, 6);
  assert.deepEqual(summary.compilationSummary, { version: "V1", apiCaseCount: 2, proposedBrowserCaseCount: 4, semanticCandidateCount: 4, rejectedSemanticCandidateCount: 0, trustedBrowserCaseCount: 0, discoveryBrowserCaseCount: 4, effectiveBrowserRuntimeCaseCount: 4, effectiveRuntimeUnitCount: 6 });
...
test("mixed lanes and zero discovery are counted without mutating the plan", () => {
  ...
  assert.equal(summarizeCompiledPlan(plan).discoveryBrowserCaseCount, 0);
});
